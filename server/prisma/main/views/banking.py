"""
Detailer bank accounts for payouts (encrypted IBAN storage).

**Auth:** ``IsAuthenticated`` — accounts scoped to the authenticated detailer's user.

**GET actions:** ``get_bank_accounts``.

**POST actions:** ``create_bank_account``.

**PATCH actions:** ``set_default_bank_account``.

**DELETE actions:** ``delete_bank_account``.
"""
...
class BankingView(APIView):
    """
    CRUD for detailer payout bank accounts (max two verified accounts).
    """
    ...
    def delete(self, request, *args, **kwargs):
        """
        Route DELETE ``action`` to handler.
    ...
    def _get_bank_accounts(self, request):
        """
        List bank accounts for the authenticated detailer.

        Args:
            request: Authenticated DRF request.

        Returns:
            JSON array of ``{id, account_name, iban, is_default}`` or empty list.
        """
        try:
            ...
            for bank_account in bank_accounts:
                bank_account_data.append({
                    ...
                    'iban': bank_account.get_iban_plain(),
    ...
    def _create_bank_account(self, request):
        """
        Add a bank account (max two verified per detailer); first account becomes primary.

        Args:
            request: Body ``bankAccountData`` with ``account_name`` and ``iban``.

        Returns:
            201 with message, or 400 if limit/validation fails.
        """
        try:
        ...
        try:
            ...
            bank_account.set_iban_plain(iban_clean)
            bank_account.save()
    ...
    def _delete_bank_account(self, request):
        """
        Remove a non-primary bank account owned by the detailer.

        Args:
            request: Body ``accountId``.

        Returns:
            Success message or 400/404 errors.
        """
        try:
            ...
            if bank_account.is_primary:
                return Response({
                    "error": "Primary bank account cannot be deleted, please set another bank account as the primary bank account first"
                }, status=status.HTTP_400_BAD_REQUEST)
    ...
    def _set_default_bank_account(self, request):
        """
        Set one account as primary and clear primary on all others.

        Args:
            request: Body ``accountId``.

        Returns:
            Success message; sends security push notification.
        """
        try:
            account_id = request.data.get('accountId')
            if not account_id:
